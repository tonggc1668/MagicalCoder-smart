...
function buildReqParam(){
    ...
    setRequestParamById(requestParam,'moduleNameFirst')
    setRequestParamById(requestParam,'moduleUrlFirst')
    setRequestParamById(requestParam,'moduleCategoryIdFirst')
    setRequestParamById(requestParam,'moduleTitleFirst')
    //排序
...
function  getItemPage(pageIndex){
    var template =
            '<tr id="tr{id}"><td><input type="checkbox" value="{id}" class="tdcheckbox"></td><td>{rankNum}</td>'+
                    ...
                    '<td>{moduleName}</td>' +
                    '<td>{moduleUrl}</td>' +
            '<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/module_category/detail_param?id={moduleCategoryId}\')"> {moduleCategoryIdForeignShowValue}</a></td>' +
                    '<td>{sortNum}</td>' +
                    '<td>{moduleTitle}</td>' +
        '<td><a href="admin/module/detail/{id}">编辑</a>' +
        '<a href="javascript:{}" onclick="deleteItem(\'module\',{id})">删除</a></td></trid>';
    var pageSize = 20;//每页多少条记录
    var pageCount = parseInt($("#pageCount").val());//总共多少条记录
    var url ='admin/module/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
...
function exportJsonFile(){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/module/export/json/'+start+'/'+limit;
