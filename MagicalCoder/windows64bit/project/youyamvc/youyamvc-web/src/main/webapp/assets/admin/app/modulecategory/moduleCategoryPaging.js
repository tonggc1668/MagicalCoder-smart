...
function buildReqParam(){
    ...
    setRequestParamById(requestParam,'moduleCategoryNameFirst')
    //排序
...
function  getItemPage(pageIndex){
    var template =
            '<tr id="tr{id}"><td><input type="checkbox" value="{id}" class="tdcheckbox"></td><td>{rankNum}</td>'+
                    ...
                    '<td>{moduleCategoryName}</td>' +
                    '<td>{sortNum}</td>' +
        '<td><a href="admin/module_category/detail/{id}">编辑</a>' +
        '<a href="javascript:{}" onclick="deleteItem(\'module_category\',{id})">删除</a></td></trid>';
    var pageSize = 20;//每页多少条记录
    var pageCount = parseInt($("#pageCount").val());//总共多少条记录
    var url ='admin/module_category/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
...
function exportJsonFile(){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/module_category/export/json/'+start+'/'+limit;
