...
function buildReqParam(){
    ...
    setRequestParamById(requestParam,'moduleIdFirst')
    setRequestParamById(requestParam,'priorityIdFirst')
    //排序
...
function  getItemPage(pageIndex){
    var template =
            ...
            '<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/module/detail_param?id={moduleId}\')"> {moduleIdForeignShowValue}</a></td>' +
            '<td><a title="点击查看详情" target="_blank" onclick="openUrl(this,\'admin/priority/detail_param?id={priorityId}\')"> {priorityIdForeignShowValue}</a></td>' +
        '<td><a href="admin/role_module_priority/detail/{id}">编辑</a>' +
        '<a href="javascript:{}" onclick="deleteItem(\'role_module_priority\',{id})">删除</a></td></trid>';
    var pageSize = 20;//每页多少条记录
    var pageCount = parseInt($("#pageCount").val());//总共多少条记录
    var url ='admin/role_module_priority/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
...
function exportJsonFile(){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/role_module_priority/export/json/'+start+'/'+limit;
