...
function buildReqParam(){
    ...
    setRequestParamById(requestParam,'registTimeFirst')
    //排序
...
function  getItemPage(pageIndex){
    var template =
        ...
        '<a href="javascript:{}" onclick="deleteItem(\'user_web\',{id})">删除</a></td></trid>';
    var pageSize = 20;//每页多少条记录
    var pageCount = parseInt($("#pageCount").val());//总共多少条记录
    var url ='admin/user_web/page/'+pageIndex+'/'+pageSize+'/'+pageCount;
    $.ajax({
        url:url,
        type:'get',
        dataType:'json',
...
function exportJsonFile(){
    var start = $("#start").val()
    var limit = $("#limit").val()
    var url ='admin/user_web/export/json/'+start+'/'+limit;
